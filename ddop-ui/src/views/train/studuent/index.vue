    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['train:studuent:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['train:studuent:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['train:studuent:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="studuentList" @selection-change="handleSelectionChange">
...
<script>
import { listStuduent, getStuduent, delStuduent, addStuduent, updateStuduent } from "@/api/train/studuent";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStuduent(this.queryParams).then(response => {
        this.studuentList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getStuduent(studentId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.studentId != null) {
            updateStuduent(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStuduent(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('train/studuent/export', {
        ...this.queryParams
      }, `studuent_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
